...
class TwoRadarFusionDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        weak_dev = self.dcfg.weak_label_device
        if worker is not None and weak_dev != "cpu":
            weak_dev = "cpu"
...
def make_loaders(sources, device, rank, world_size, num_workers, prefetch_factor):
    ...
    if weak_dev != "cpu" and num_workers != 0 and is_main(rank):
        print(
            "⚠️  weak-label device set to GPU: forcing DataLoader workers to 0 "
            "so CUDA runs in the main process.",
            flush=True,
        )
    worker_count = 0 if (weak_dev != "cpu" and num_workers != 0) else num_workers
    ...
    def mk(ds, bs, shuffle, sampler):
        kwargs = dict(dataset=ds, batch_size=bs, shuffle=shuffle, sampler=sampler,
                      num_workers=worker_count, pin_memory=pin, worker_init_fn=_wif,
